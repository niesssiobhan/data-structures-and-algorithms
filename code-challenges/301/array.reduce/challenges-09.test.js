...
const returnNames = (arr) => {
  // Solution code here...
  let peopleNames = arr.reduce((names, character) => {
...
const reversedString = (arr) => {
  ...
  let string = newString.reduce((newString, currentLetter) => {
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let kidCount = arr.reduce((accumulator, value, idx) => {
    console.log('IDX', idx, 'Accumulator:', accumulator)
    if(value.children === []) {accumulator++;}
    return accumulator
  }, 0);
  return kidCount
};
