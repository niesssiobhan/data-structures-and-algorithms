...
const totalSum = (input) => {
  // Solution code here...
  let totalNum = input.map(input => input.reduce((sum, item) => sum += item, 0));
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newNums = input.filter(nums => nums % 5);
  let totalNums = [];
  for( let i = 0; i < newNums.length; i++){
    let exponent = newNums[i]
    base = 
    totalNums.push(Math.pow(exponent, base))
  }
  return totalNums
};
